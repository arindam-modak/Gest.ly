...
def test(model):
    ...
    while True:
        ...
        # cv2.imwrite(os.path.join(folder,"frame{:d}.jpg".format(count)), res) 
        # count += 1
        res1 = cv2.resize(res, (100,100))
        print(gesCNN.guessGesture(model, res1))
        k = cv2.waitKey(5) & 0xFF
        if k==27:
            break
    cap.release()
